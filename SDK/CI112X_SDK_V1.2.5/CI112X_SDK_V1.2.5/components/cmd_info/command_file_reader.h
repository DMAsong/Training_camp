...
typedef struct command_info_st
{
	...
	uint16_t voice_index;			//!Index of this command's voice in voice table.
...
{
	uint16_t command_id;			//!Identity of command.
	int8_t wait_count;				//!Time wait after special word.
 ...
 * @return uint32_t 0:sucessful, others:error number.
 */
extern uint32_t cmd_file_get_cur_model_id(uint32_t *p_dnn_id, uint32_t *p_asr_id, uint32_t *p_voice_group_id);
