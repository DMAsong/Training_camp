...
extern  int alg_res_compute_set_asr_mode( short *mic_data,  short *dst_data, int frame_length,int mode_change_enable );
extern int apply_fe(float *fft_data);
extern int deal_vad_state(int state,short*);
...
/**************************************************************************
                    function
****************************************************************************/
static int alg_fft_ifft_init( ci_fft_ifft_creat_t* S )
{
    ci_logdebug(LOG_AUDIO_IN,"\n-----%d\n",xPortGetFreeHeapSize());
    int ret = 0;
    ret = ci_fft_ifft_test_create( S );
    if (ret != 0)
    {
        ci_logerr(LOG_AUDIO_IN,"%s().%d: ci_fft_ifft_test_create() failed, exit\n", __FUNCTION__, __LINE__ );
        return -1;
    }
    ci_logdebug(LOG_AUDIO_IN,"\n-----%d\n", xPortGetFreeHeapSize() );
...
static int alg_aec_init( void )
{
//    ci_logdebug(LOG_AUDIO_IN,"\n-----%d\n", xPortGetFreeHeapSize() );
//
//    adapt_aec = ci_adapt_aec_create( ADAPT_AEC_MODE_PCMIN_PCMOUT, FFT_SIZE, MIC_CHANNEL_NUM, REF_CHANNEL_NUM );
//    ci_adapt_aec_threshold_set( adapt_aec, AEC_THRESHOLD_REF_ENG_TH, AEC_THRESHOLD_SET_ENABLE );
//
//    if (!adapt_aec)
//    {
//        ci_logerr(LOG_AUDIO_IN,"%s().%d: ci_adapt_aec_create() failed, exit\n", __FUNCTION__, __LINE__ );
//        return -1;
//    }
//    ci_logdebug(LOG_AUDIO_IN,"\n-----%d\n", xPortGetFreeHeapSize() );
...
static int alg_tdoa_init( void )
{
//    ci_logdebug(LOG_AUDIO_IN,"\n-----%d\n", xPortGetFreeHeapSize() );
//
//    tdoa = (void*)ci_tdoa_create( 40 );
//    if (!tdoa)
//    {
//        ci_logerr(LOG_AUDIO_IN,"%s().%d: ci_tdoa_create() failed, exit\n", __FUNCTION__, __LINE__ );
//        return -1;
//    }
//    ci_logdebug(LOG_AUDIO_IN,"\n-----%d\n", xPortGetFreeHeapSize() );
...
//    if (!bf)
//    {
//        ci_logerr(LOG_AUDIO_IN,"%s().%d: ci_bf_create() failed, exit\n", __FUNCTION__, __LINE__ );
//        return -1;
//    }
//    ci_logdebug(LOG_AUDIO_IN,"\n-----%d\n", xPortGetFreeHeapSize() );
//
//#if CI_BF_CONFIG_BEAMWIDTH
//    beamwidth_e beamwidth = BEAMWIDTH_L;
//    if ( ci_bf_set_beamwidth( bf, beamwidth ) != 0 )
//    {
//        ci_logerr(LOG_AUDIO_IN,"%s().%d: ci_bf_set_beamwidth() failed, exit\n", __FUNCTION__, __LINE__ );
//        return -1;
//    }
//#endif
...
static int iir_hpf_init( void )
{
//    ci_logdebug(LOG_AUDIO_IN,"\n-----%d\n", xPortGetFreeHeapSize() );
//
//    iir_hpf_l = arm_biquad_cascade_df1_init_f32_iir_hpf();
//    iir_hpf_r = arm_biquad_cascade_df1_init_f32_iir_hpf();
//    if ( (!iir_hpf_l) || (!iir_hpf_r) )
//    {
//        ci_logerr(LOG_AUDIO_IN,"%s().%d: arm_biquad_cascade_df1_init_f32_iir_hpf() failed, exit\n", __FUNCTION__, __LINE__ );
//        return -1;
//    }
//    ci_logdebug(LOG_AUDIO_IN,"\n-----%d\n", xPortGetFreeHeapSize() );
...
static int alg_warpfft_deal_model(void)
{
    ...
    wrapfft_audio_t =  ci_warpfft_create( mic_channel_num,ref_channel_num,800, 0);
