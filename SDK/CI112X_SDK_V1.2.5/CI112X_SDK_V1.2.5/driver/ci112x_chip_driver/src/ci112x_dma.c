...
dma_callback_func_ptr_t g_dma_channel0_callback = NULL;
...
{    
    ...
    DMAC_ChannelSourceConfig(dmachannel,INCREMENT,DMAC_AHBMaster1,\
		TRANSFERWIDTH_32b,BURSTSIZE16);
    DMAC_ChannelDestConfig(dmachannel,INCREMENT,DMAC_AHBMaster2,\
		TRANSFERWIDTH_32b,BURSTSIZE16);
...
void DMAC_M2P_P2M_advance_config(DMACChannelx dmachannel,DMAC_Peripherals periph,\
  DMAC_FLOWCTRL flowctrl,unsigned int srcaddr,unsigned int destaddr,\
	unsigned int bytesize,TRANSFERWIDTHx datawidth,BURSTSIZEx burstsize)
{
...
void spic_dma_config(DMACChannelx channel,dma_config_t* config)
{
    ...
    DMAC_ChannelSourceConfig(channel,srcinc,DMAC_AHBMaster1,
                             config->transferwidth,
                             config->busrtsize);
    DMAC_ChannelDestConfig(channel,destinc,DMAC_AHBMaster2,
                           config->transferwidth,
                           config->busrtsize);
